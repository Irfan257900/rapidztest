...
const TransactionChart = ({userInfo}) => {
    const chartProps = {
        type: "column",
    ...
    const getTransactionsChartData = useCallback(() => {
        return appClientMethods.get(`${ApiControllers.dashboard}MainDashboardGraph/${filters ||"7"}`);
      }, [filters]);
    ...
    return (
        <div className="border border-dbkpiStroke p-2.5 md:p-5 rounded-5"><div className="md:flex block items-start justify-between pb-5 md:justify-end">
            <h3 className="text-2xl font-semibold text-titleColor mb-2 md:hidden block">Transactions Summary</h3>
               <div className="text-right">
                <div className="range-buttons flex md:block border border-dbkpiStroke rounded-md">
