...
const apiClient = createApiClient(window.runtimeConfig.VITE_API_END_POINT);
...
const createRequest = (client) => ({
    ...
    post: async (url, data) => handleResponse(await client.post(url, data)),
    put: async (url, data) => handleResponse(await client.put(url, data)),
    delete: async (url) => handleResponse(await client.delete(url)),
});
