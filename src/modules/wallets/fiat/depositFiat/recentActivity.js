...
import { FaitTransactions } from "../../api/services";
import { setRefreshTransactionGrid } from "../withdraw.components/withdrawFiatReducer";
import { StateHandler } from "../../crypto/service";
import AppEmpty from "../../../../core/shared/appEmpty";
import NumericText from "../../../../core/shared/numericText";

const RecentActivityFait = ({ actionType }) => {
    const navigate = useNavigate();
    const { t } = useTranslation();
    const [transactions, setTransactions] = useState([]);
    const [loading, setLoading] = useState(false);
    const [errorMsg, setErrorMsg] = useState(null);
    const isRefresh = useSelector((storeInfo) => storeInfo?.withdrawFiat?.isRefreshTransactionGrid);
    ...
    useEffect(() => {
        if (isRefresh) {
            fetchTransactions();
            dispatch(setRefreshTransactionGrid(false));
        }
    }, [isRefresh])
    ...
    const fetchTransactions = async () => {
        setLoading(true);
        try {
            await FaitTransactions(actionType, setLoading, setTransactions, setErrorMsg);
        } catch (error) {
...
export default RecentActivityFait;
