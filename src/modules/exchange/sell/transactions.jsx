...
const Transactions = () => {
  const selectedCryptoCoin=useSelector(store=>store.sellState.selectedCryptoCoin)
  const {loader}=useSelector(store=>store.sellState.cryptoCoins)
...
const customFeeCell = (propsData) => {
  ...
  const listUrl = useMemo(() => {
    return `${baseURL}sell/transactions`;
  }, [baseURL]);
  ...
  return (
    <div className="">
        <div className="flex justify-between items-center mb-2">
          ...
          <Link
            to="/transactions?module=Exchange&action=Sell"
