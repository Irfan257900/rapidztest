...
import { amountValidation, BUY_VALIDATION_MESSAGES } from "./validation";
import { replaceCommas } from "../../../core/shared/validations";
import { buyCoinReducer, buyCoinState } from "./reducers";
...
function Buy({
  ...
  const [state, setState] = useReducer(buyCoinReducer, buyCoinState);
  const dispatch = useDispatch();
  const {
    loading: fetchingAssets,
    error: assetsError,
    data: assets,
  } = useSelector((state) => state.buyState.widgetAssets);

  const {
    loading: fetchingAssetDetails,
    data: assetDetails,
    error: assetError,
  } = useSelector((state) => state.buyState.assetDetails);

  const selectedFiatCoin = useSelector(
    (state) => state.buyState.selectedFiatCoin
  );
  ...
  const getSummary = async () => {
    try {
      ...
      const summary = await fetchBuySummary({
  ...
  const handleBuy = useCallback(() => {
    ...
    if (!selectedFiatCoin) {
      setError(BUY_VALIDATION_MESSAGES.NO_FIAT_WALLET);
      setState({ type: "setLoading", payload: "" });
      return;
    }
  ...
  const handleCryptoCurrencyChange = useCallback(
    (value) => {
      ...
      if (selected) {
        ...
        fetchAssetDetailsOnChange &&
          getAssetDetails({ assetCode: selected.code });
        ...
        <div className="panel-card buy-card card-paddingrm">
          <BuySellWidget
            ...
            config={{
              ...
              selectedCryptoCoin: assetDetails,
              coinsDropdownPlaceholder: "Select coin to buy",
