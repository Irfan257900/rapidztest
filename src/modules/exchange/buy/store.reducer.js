...
export const fetchCryptoCoins = createAsyncThunk(
  'buy/fetchCryptoCoins',
  async (params, { rejectWithValue, getState }) => {
    const { page = 1, pageSize, data } = params || {}
    const state = getState()
    const { pageSize: defaultPageSize } = state.buyState.cryptoCoins
    try {
...
export const fetchAssetDetails = createAsyncThunk(
  'buy/fetchAssetDetails',
  async ({ assets, assetCode, onSuccess }, { rejectWithValue, dispatch }) => {
    try {
      const response = await appClientMethods.get(`assets/buy/${assetCode}`);
...
export const fetchAssetsAndBalances = createAsyncThunk(
  'buy/fetchAssetsAndBalances',
  async (_, { rejectWithValue }) => {
    try {
      return await appClientMethods.get(`assets/buy/balances`);
    } catch (error) {
      return rejectWithValue(error.message);
    }
  }
...
const initialState = {
  summaryDetails: null,
  cryptoCoins: { loader: 'init', data: null, error: '', pageSize: 20, page: 1 },
  refreshData: false,
  saveResponse: null,
...
const buySlice = createSlice({
  ...
  reducers: {
    ...
    setDataForSummary(state, action) {
      state.dataForSummary = action.payload;
    },
...
export const { setErrorMessages, resetState, setRefreshData, setSummaryDetails, setDataForSummary, clearCryptoCoinsError, setSelectedCryptoCoin, setSelectedFiatCoin, setSaveResponse } = buySlice.actions;
