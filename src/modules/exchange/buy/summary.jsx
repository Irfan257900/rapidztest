...
import { handleBuyCrypto } from "../http.services";
...
const Summary = ({onSuccess,onError,onClose,classNames={},isCrypto}) => {
  const buyDivRef = React.useRef(null);
  ...
  const summaryDetails = useSelector((state) => state.buyState.summaryDetails);
  ...
  return (
    <>
      ...
      {loader !== "success" && <>
        ...
        <AppTitle
          level={2}
          className=" text-center xl:!text-base  !text-titleColor font-semibold"
        >
          Summary
        </AppTitle>
        <SummaryDetails details={summaryDetails} flag={isCrypto} action="Buy" />
        <CustomButton
          block
          type="primary"
        ...
        >
          <span className="">Confirm {summaryDetails?.toAsset}</span>
        </CustomButton>
      </div>
      {note && (
        <AppParagraph className="note-text">
          ...
          >
            Note :&nbsp;<span className="text-labelGrey text-sm font-normal">{note}</span>
          </AppTitle>
